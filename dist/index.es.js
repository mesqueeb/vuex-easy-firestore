...
import { isObject, isArray, isString } from 'is-what';
...
var mutations = {
  ...
  PATCH_DOC: function PATCH_DOC(state, doc) {
    ...
    if (state.firestoreRefType.toLowerCase() === 'doc') {
      if (!state.docsStateProp) {
        return Object.keys(doc).forEach(function (key) {
          // Merge if exists
          var newVal = state[key] === undefined ? doc[key] : !isObject(state[key]) || !isObject(doc[key]) ? doc[key] : merge(state[key], doc[key], { arrayOverwrite: true });
    ...
    // Merge if exists
    var newVal = state[state.docsStateProp][doc.id] === undefined ? doc : !isObject(state[state.docsStateProp][doc.id]) || !isObject(doc) ? doc : merge(state[state.docsStateProp][doc.id], doc, { arrayOverwrite: true });
...
var getters = {
  signedIn: function signedIn(state, getters, rootState, rootGetters) {
    var user = getDeepRef(rootState, state.vuexUserPath);
    return user !== null;
  },
...
function createEasyFirestore(userConfig) {
  return function (store) {
    ...
    userConfig.forEach(function (config) {
      var moduleNameSpace = getKeysFromPath(config.moduleNameSpace);
