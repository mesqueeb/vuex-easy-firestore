...
var mutations = {
  ...
  PATCH_DOC: function PATCH_DOC(state, doc) {
    ...
    return Object.keys(doc).forEach(function (key) {
      // Merge if exists
      var newVal = isObject(ref[key]) && isObject(doc[key]) ? merge(ref[key], doc[key]) : doc[key];
...
function retrievePaths(object, path, result) {
  if (!isObject(object) || !Object.keys(object).length || object.methodName === 'FieldValue.serverTimestamp') {
...
var actions = {
  ...
  deleteDoc: function deleteDoc(_ref3) {
    ...
    // 0. payload correction (only arrays)
    if (!isArray(ids)) ids = [ids]; // 1. Prepare for patching
    // 2. Push to syncStack
  ...
  openDBChannel: function openDBChannel(_ref17, pathVariables) {
    ...
    if (pathVariables && isObject(pathVariables)) commit('SET_PATHVARS', pathVariables); // get userId
...
function errorCheck(config) {
  ...
  numberProps.forEach(function (prop) {
    ...
    if (!isNumber(_prop)) errors.push("`".concat(prop, "` should be a Number, but is not."));
  });
  ...
  functionProps.forEach(function (prop) {
    ...
    if (!isFunction(_prop)) errors.push("`".concat(prop, "` should be a Function, but is not."));
  });
  ...
  objectProps.forEach(function (prop) {
    ...
    if (!isObject(_prop)) errors.push("`".concat(prop, "` should be an Object, but is not."));
  });
  ...
  stringProps.forEach(function (prop) {
    ...
    if (!isString(_prop)) errors.push("`".concat(prop, "` should be a String, but is not."));
  });
  var arrayProps = ['where', 'orderBy', 'fillables', 'guard'];
  arrayProps.forEach(function (prop) {
    var _prop = config.sync[prop];
    if (!isArray(_prop)) errors.push("`".concat(prop, "` should be an Array, but is not."));
  });
...
function index (userConfig) {
  return function (store) {
    ...
    if (!isArray(userConfig)) userConfig = [userConfig]; // Create a module for each config file
